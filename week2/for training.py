...
class TestPodcast:
    texts_search_thematic_matching = [
        "включи подкаст про психологию" "включи подкаст про спорт",
    ...
    def test_search_thematic_matching(self, text):
        ...
        data = {
            "session_id": "f41fe49a-393d-4e56-b1d4-4e23b21e0958",
    ...
    def test_discover_choice(self):
        ...
        while (
            text.find("политику") == -1 and try_count <= expexted_count
        ):  # пока в выдаче не попадется политику
            # спрашиваем снова эту тему
            response = requests.post(url, params=data)
            parced_response_text = response.json()
            text = parced_response_text["result"]["phrase_result"]["commands"][0][
                "text"
            ]  # выбор сначала по ключу,
            # а потом по индексу из поиска
            try_count = try_count + 1
        assert (
            try_count <= expexted_count
        ), f"За {expexted_count} попыток не попалась тема политика"
        data = {
            "session_id": "f41fe49a-393d-4e56-b1d4-4e23b21e0958",
            "device_id": ":c:m:android:23fc82a4e6c146b7a2730bba66924358",
            "skillserver_type": "skillteam",
        ...
        response = requests.post(
            url, params=data
        )  # теперь уже выбираем тему "политика из предложенных"
        parced_response_text1 = response.json()
        text = parced_response_text1["result"]["phrase_result"]["commands"][0]["text"]
        assert (
            text.find("Слушаем подкаст") != -1
        )  # оказалось, что -1 это не нашлось, то есть тут проверка отрицания,
        # что ничего не нашлось


# get_intent = parced_response_text.get('result').get('phrase_result').get('intent')
# get_intent = parced_response_text.get('result').get('phrase_result').get('commands')[0]


# @pytest.mark.skip
# @pytest.mark.parametrize('verb', LAUNCH_WORDS)
# def test_podcasts_run(verb, front):
#     result = front.text(verb + ' подкаст')
#     assert result['intent'] == 'podcasts_v2.random'
#
#     result = front.text(verb + ' подкасты')
#     assert result['intent'] == 'podcasts_v2.random'
